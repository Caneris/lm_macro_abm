...
def refin_firms(def_firms, surviving_firms, h_arr, n_refin, tol, t):
    ...
    for id in rand_ids:
        ...
        if wealth_tot >= 0:
            ...
            def_firms[id].s_e = np.maximum(s_e_init + rd.randn()*sigma_s_e, 0.001)
            def_firms[id].m = np.maximum(m_init + rd.randn()*sigma_m, 0.001)
            def_firms[id].Wr_e = np.maximum(Wr_e_init + rd.randn() * sigma_Wr_e, 0.001)
            def_firms[id].Wnr_e = np.maximum(Wnr_e_init + rd.randn() * sigma_Wnr_e, 0.001)
            n_refin[t] += 1
